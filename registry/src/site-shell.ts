...
/* The mark: the wireframe hand from src/shared/brand-mark.ts — the same three
   paths the app draws in CrLogoMark.tsx. Ink here, on the cream header. */
const LOGO = markSvg({ stroke: 2, attrs: 'aria-hidden="true"' })
...
/* the lockup on its phosphor band (site-home.ts brandBand): cyan wireframe,
   CSS only. 100 units = 1em of --wm. */
...
.lk .c{width:calc(var(--wm) * 1.5);height:calc(var(--wm) * 1.5);margin:calc(var(--wm) * -.3) 0 calc(var(--wm) * -.2) 0;flex:0 0 auto;position:relative;z-index:1;filter:drop-shadow(0 0 .14em rgba(62,232,224,.5))}
.lk .c svg,.lk .h svg{width:100%;height:100%;display:block}
.lk .o{position:relative;z-index:1;width:calc(var(--wm) * .82);height:calc(var(--wm) * .82);border-radius:50%;border:.035em solid currentColor;background:#07191b;display:grid;place-items:center;flex:0 0 auto;box-shadow:0 0 .2em rgba(62,232,224,.35)}
.lk .o::before{content:'';position:absolute;inset:.07em;border-radius:50%;border:1px dashed currentColor;opacity:.5}
.lk .p{width:34%;height:34%;border-radius:50%;background:#dffcfa;box-shadow:0 0 .12em currentColor;animation:lk-glance 6s cubic-bezier(.6,0,.2,1) infinite}
...
.lk .h{position:absolute;left:.02em;bottom:-.8em;width:.8em;height:.66em;animation:lk-crab 6s cubic-bezier(.5,0,.5,1) infinite;filter:drop-shadow(0 0 .1em rgba(62,232,224,.5))}
...
@keyframes lk-glance{0%,6%{transform:translate(0,0)}10%,46%{transform:translate(24%,10%)}50%,62%{transform:translate(30%,0)}66%,84%{transform:translate(-4%,10%)}90%,100%{transform:translate(0,0)}}
