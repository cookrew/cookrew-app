...
// V4 §3 — the dispatch route: attach-free agent-to-agent work.
//
...
function deps(over: Partial<DispatchDeps> = {}): DispatchDeps {
  return {
    resolveAgent: (id) =>
      id === 'agent-1' ? { name: 'Forge', workspaceId: 'ws-hot' } : null,
    isHot: (ws) => ws === 'ws-hot',
...
async function dispatchAndSettle(
  ...
  agentId = 'agent-1',
  body: { text?: string; brief?: string; idempotencyKey?: string } = { text: PROMPT }
...
// ---------------------------------------------------------------------------
// Refusals, in the order §3 states them.
// ---------------------------------------------------------------------------
...
describe('POST /api/agents/:id/dispatch — refusals', () => {
  ...
  it('409s DORMANT when the target workspace is not hot', async () => {
    let calls = 0
    const service = new DispatchService(
      deps({
        isHot: () => false,
    ...
    expect(response.body).toMatchObject({ error: 'dormant' })
    expect(calls).toBe(0)
  })
  ...
  it('refuses the dispatch outright when the tracker still holds a live stamp', async () => {
    ...
    // Refused BEFORE the record is written — an unbillable request costs
...
describe('F2 — a stalled report is confirmed, never retried blind', () => {
  ...
  it('logs a failed submission WITHOUT the brief it was carrying (F16)', () => {
    ...
    // <pane> <prompt>` puts the consumer's entire paid brief in the app log
    // and in the record's error field, which is served over HTTP.
...
describe('~/.cookrew/dispatches.jsonl — append only', () => {
  ...
  it('never writes the agent’s reply to disk — only that there was one', () => {
    ...
    // billing and correlation trace, not a second transcript store with its
    // own lifetime and no reader. turnIndex points at the turn that has it.
    const file = path.join(mkdtempSync(path.join(tmpdir(), 'cookrew-dsp-')), 'dispatches.jsonl')
    appendDispatchRecord(
      file,
...
describe('sweep — nothing holds an agent’s slot forever', () => {
  ...
  it('stamps a dispatch whose turn never completed, and frees the agent', async () => {
    // The gap release() could not close: only a completed turn, an app quit or
    // a workspace leaving HOT reach the terminal edge. An agent killed
    // mid-turn reaches none of them, so its slot answered 409 busy forever.
...
// ---------------------------------------------------------------------------
// F5 — the ledger is the state, so a restart is not a billing event.
// ---------------------------------------------------------------------------
...
describe('a restart rehydrates from the ledger', () => {
  ...
  it('replays the same idempotencyKey instead of dispatching (and billing) again', async () => {
    const file = ledger()
    let prompts = 0
    ...
    // The process dies. The consumer retries the key it already paid for.
  ...
  it('stamps a row that was still open INTERRUPTED, not failed', async () => {
    ...
    // `failed` would say "we never delivered this", which is a lie a consumer
...
// ---------------------------------------------------------------------------
// F8/F9 — nothing is left holding a slot or an armed stamp.
// ---------------------------------------------------------------------------
...
describe('an ended dispatch lets go of the agent', () => {
  it('disarms the tracker when the dispatch ends without a turn (F8)', async () => {
    ...
    // — and billed — against a dispatch nobody is waiting on.
  ...
  it('interrupts the open dispatches of a workspace leaving HOT (F9)', async () => {
    const service = new DispatchService(deps())
    await dispatchAndSettle(service)
    expect(service.openDispatchIds()).toEqual(['dsp-1'])

    expect(service.interruptWorkspace('ws-other', 'went dormant')).toEqual([])
    expect(service.interruptWorkspace('ws-hot', 'went dormant')).toEqual(['dsp-1'])
    expect(service.get('dsp-1')).toMatchObject({ state: 'interrupted' })
    // The slot came back with it — a dead dispatch cannot hold an agent.
    expect(service.openDispatchIds()).toEqual([])
  })
...
describe('the routes themselves', () => {
  ...
  it('401s the GET without a token — the write gate never covered it (F4)', async () => {
    ...
    // 0.0.0.0 listener: any id, from anyone on the LAN, described paid work at
    // a named agent. Same exposure argument as /api/board, same gate.
    const service = new DispatchService(deps())
    await dispatchAndSettle(service)
    const { response, captured } = stubResponse()
    const handled = await handleMobileApi(
  ...
  it('serves the GET to the pairing token, and 403s the read-only one', async () => {
    // The manifest puts this route in `dispatch`, not `observe`: following a
    // dispatch is part of commissioning work, and the wall's token is scoped to
    // the curated read projections. 403 rather than 401 — the token is known.
    const service = new DispatchService(deps())
    await dispatchAndSettle(service)
...
describe('dispatchId rides the turn it caused', () => {
  ...
  it('stamps the next completed turn and only that one', async () => {
    ...
    // and billing a caller for it would be a fabricated correlation.
  ...
  it('closes a detached dispatch only after its mutable session record is final', async () => {
    ...
    const service = new DispatchService(
      deps({
        noteDispatch: (agentId, dispatchId) => tracker.noteDispatch(agentId, dispatchId),
        clearDispatch: (agentId, dispatchId) => tracker.clearDispatch(agentId, dispatchId),
        promptAgent: async () => 'done'
      })
    )
    tracker.on('turn', ({ terminalId, dispatchId, turnIndex }: CompletedTurn) => {
      if (dispatchId === undefined) return
      const turn = [...tracker.history(terminalId)]
        .reverse()
        .find((record) => record.index === turnIndex)
      service.completeTurn(dispatchId, {
        turnIndex: turn?.index ?? turnIndex ?? 0,
    ...
    tracker.replaceHistory('agent-1', [
      { ...promptOnly, reply: 'counts reported', endedAt: 18_000 }
    ])
    expect(service.get('dsp-1')?.state).toBe('running')
