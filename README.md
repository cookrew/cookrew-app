...
An open-source spatial workspace for AI agents, inspired by [Cookrew](https://www.cookrew.dev).
...
Built with Electron + React, so it runs beyond macOS (the original Cookrew is a native
...
- **Notes** — markdown sticky notes, auto-named from their first line (rename-on-edit,
  Cookrew-style), persisted as real `.md` files under `~/.cookrew/notes`
...
- **ask engine** — prompts are written into the target PTY; a headless xterm mirrors
  ...
  produced text (same blocking semantics as `cookrew ask`)
...
Design choices mirroring Cookrew:
