/**
 * The 2048 words a key fingerprint is spoken in.
 *
 * THE WORDS ARE THE SECURITY (Velvet's deck, §3). The fingerprint comparison
 * happens between two humans over a channel we do not control — a phone call, a
 * video call, a room. Six words can be read aloud in three seconds and confirmed
 * with certainty; sixteen hex characters are read aloud wrongly. So the list is
 * not decoration and its properties are load-bearing:
 *
 *   EXACTLY 2048, so each word carries exactly 11 bits and six carry 66.
 *   ALL DISTINCT, or two different keys could speak the same phrase.
 *   UNAMBIGUOUS AT FOUR CHARACTERS, so a word misheard is still recoverable.
 *   3-8 CHARACTERS, lowercase a-z only — nothing to spell out or transliterate.
 *
 * This is the BIP-39 English wordlist, which was designed for this exact job and
 * has had far more adversarial attention than anything written here would get.
 * It is VENDORED rather than depended on: a fingerprint list that could change
 * under a package update would silently invalidate every comparison an owner has
 * ever made. Verified at vendoring time — 2048 lines, 2048 distinct, 2048
 * distinct four-character prefixes, sha256
 * 2f5eed53a4727b4bf8880d8f3f199efc90e58503646d9ff8eff3a2ed3b24dbda — and
 * re-checked on every run by tests/key-fingerprint.test.ts, because a list that
 * quietly lost a word would weaken every fingerprint without erroring.
 *
 * Generated, not hand-authored. Do not edit by hand.
 */
